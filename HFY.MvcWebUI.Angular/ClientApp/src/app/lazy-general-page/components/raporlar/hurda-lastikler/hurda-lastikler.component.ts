...
@Component({
  selector: 'app-hurda-lastikler',
  templateUrl: './hurda-lastikler.component.html',
  styleUrls: ['./hurda-lastikler.component.css']
})
export class HurdaLastiklerComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private route: ActivatedRoute, private authorize: AuthorizationService, 
    ...
    this.subscription = this.dataTablesRxJs.getMessage().subscribe(message => {
      if (message.componentRequest == "Lastik/HurdaLastikler") this.dataTablesData = message.data
    });
  ...
  ngOnInit() {
    this.dtOptions = this.dataTableService.getData("Lastik/HurdaLastikler", 10, new Lastikler().jqueryDataTableLastikler());
  }
