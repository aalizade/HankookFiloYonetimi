import { Component, AfterViewInit, Input, ViewChild, ViewContainerRef, ComponentFactoryResolver, Output, EventEmitter } from '@angular/core';
...
@Component({
  selector: 'app-a-k-olcumle',
  templateUrl: './a-k-olcumle.component.html',
  styleUrls: ['./a-k-olcumle.component.css']
})
export class AKOlcumleComponent implements AfterViewInit {
  ...
  @ViewChild('KopyalaContainer_Olcumle', { read: ViewContainerRef, static: false }) kopyalaContainer: ViewContainerRef;
  ...
  async olcumleOpen(form: { ServisTarihi: string, AracKilometre: number }) {
    ...
    if ((new Date(form.ServisTarihi) < new Date(this.sonServisTarihi)) || (new Date() < new Date(form.ServisTarihi))) {
          ...
          let sorunAra3 = await this.lastikHareketlerService.getLastikHareketlerWithArac(item.LastikID, this.aracId).toPromise().then(async lastikHareketResponse => {
            ...
            if (this.lastikId === item.LastikID) {
              ...
              try {
                ...
                else if ((new Date(form.ServisTarihi) < new Date(sonServisTarihi)) || (new Date() < new Date(form.ServisTarihi))) {
                  this.toastr.error("Servis tarihi, önceki servis tarihinden küçük olamaz. Günümüz tarihinden'de büyük olamaz.");
                  trueFalse = false;
                }
              }
      ...
      this.servisTarihi = form.ServisTarihi;
  ...
  onFormSubmit(form: LastikOlcumEkle) {
    ...
    this.lastikOlcumService.lastikOlcumEkle(form).subscribe(async response => {
      if (response.MessageType === 1) {
        ...
        this.aracBakimItemSuruklenenGet.ServisTarihi = this.servisTarihi;
  ...
  gozlemYap() {
    ...
    componentRef.instance.servisTarihi = this.servisTarihi;
  ...
  lastikKopyala() {
    ...
    componentRef.instance.lastikId = this.lastikId;
    componentRef.instance.lastikKopyalaAktif = true;
    componentRef.instance.lastikPozisyonId = this.aracBakimItemSuruklenenLastikPozisyonId;

    const sub: Subscription = componentRef.instance.kaydet.subscribe(returnValue => {
