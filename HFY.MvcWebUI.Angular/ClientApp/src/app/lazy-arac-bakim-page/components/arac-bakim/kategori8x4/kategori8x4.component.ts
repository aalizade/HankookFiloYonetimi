...
@Component({
  selector: 'app-kategori8x4',
  templateUrl: './kategori8x4.component.html',
  styleUrls: ['./kategori8x4.component.css']
})
export class Kategori8x4Component implements OnInit {
  ...
  aks212ASL: AracBakimlar[] = []; aks212ASLName: string = "212-A-SL"; aks212ASLId: number;
  aks23ASG: AracBakimlar[] = []; aks23ASGName: string = "23-A-SG"; aks23ASGId: number;

  aks310ASLDC: AracBakimlar[] = []; aks310ASLDCName: string = "310-A-SLDC"; aks310ASLDCId: number;
  aks34ASGDC: AracBakimlar[] = []; aks34ASGDCName: string = "34-A-SGDC"; aks34ASGDCId: number;
  aks311ASLIC: AracBakimlar[] = []; aks311ASLICName: string = "311-A-SLİC"; aks311ASLICId: number;
  aks35ASGIC: AracBakimlar[] = []; aks35ASGICName: string = "35-A-SGİC"; aks35ASGICId: number;

  aks48ASLDC: AracBakimlar[] = []; aks48ASLDCName: string = "48-A-SLDC"; aks48ASLDCId: number;
  aks46ASGDC: AracBakimlar[] = []; aks46ASGDCName: string = "46-A-SGDC"; aks46ASGDCId: number;
  aks49ASLIC: AracBakimlar[] = []; aks49ASLICName: string = "49-A-SLİC"; aks49ASLICId: number;
  aks47ASGIC: AracBakimlar[] = []; aks47ASGICName: string = "47-A-SGİC"; aks47ASGICId: number;
                ...
                if (aracKategoriResponse.Ad !== this.pageName) {
                  ...
                  this.aksPozisyonlarService.getAksPozisyonlar().toPromise().then(aksPozisyonResponse => {
                    ...
                    try {
                      ...
                      this.aks212ASLId = aksPozisyonResponse.find(a => a.Ad === this.aks212ASLName).AksPozisyonID;
                      this.aks23ASGId = aksPozisyonResponse.find(a => a.Ad === this.aks23ASGName).AksPozisyonID;

                      this.aks310ASLDCId = aksPozisyonResponse.find(a => a.Ad === this.aks310ASLDCName).AksPozisyonID;
                      this.aks34ASGDCId = aksPozisyonResponse.find(a => a.Ad === this.aks34ASGDCName).AksPozisyonID;
                      this.aks311ASLICId = aksPozisyonResponse.find(a => a.Ad === this.aks311ASLICName).AksPozisyonID;
                      this.aks35ASGICId = aksPozisyonResponse.find(a => a.Ad === this.aks35ASGICName).AksPozisyonID;

                      this.aks48ASLDCId = aksPozisyonResponse.find(a => a.Ad === this.aks48ASLDCName).AksPozisyonID;
                      this.aks46ASGDCId = aksPozisyonResponse.find(a => a.Ad === this.aks46ASGDCName).AksPozisyonID;
                      this.aks49ASLICId = aksPozisyonResponse.find(a => a.Ad === this.aks49ASLICName).AksPozisyonID;
                      this.aks47ASGICId = aksPozisyonResponse.find(a => a.Ad === this.aks47ASGICName).AksPozisyonID;
                      ...
                      this.aracBakimlarService.getAracBakimlarForAracId(response.AracID).subscribe(aracBakimResponse => {
                        ...
                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks212ASLId && a.Aktif === true) === undefined) this.aks212ASL.push();
                        else this.aks212ASL.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks212ASLId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks23ASGId && a.Aktif === true) === undefined) this.aks23ASG.push();
                        else this.aks23ASG.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks23ASGId && a.Aktif === true));



                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks310ASLDCId && a.Aktif === true) === undefined) this.aks310ASLDC.push();
                        else this.aks310ASLDC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks310ASLDCId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks34ASGDCId && a.Aktif === true) === undefined) this.aks34ASGDC.push();
                        else this.aks34ASGDC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks34ASGDCId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks311ASLICId && a.Aktif === true) === undefined) this.aks311ASLIC.push();
                        else this.aks311ASLIC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks311ASLICId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks35ASGICId && a.Aktif === true) === undefined) this.aks35ASGIC.push();
                        else this.aks35ASGIC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks35ASGICId && a.Aktif === true));




                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks48ASLDCId && a.Aktif === true) === undefined) this.aks48ASLDC.push();
                        else this.aks48ASLDC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks48ASLDCId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks46ASGDCId && a.Aktif === true) === undefined) this.aks46ASGDC.push();
                        else this.aks46ASGDC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks46ASGDCId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks49ASLICId && a.Aktif === true) === undefined) this.aks49ASLIC.push();
                        else this.aks49ASLIC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks49ASLICId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks47ASGICId && a.Aktif === true) === undefined) this.aks47ASGIC.push();
                        else this.aks47ASGIC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks47ASGICId && a.Aktif === true));
  ...
  aracBakimIslemleriKaydet() {
    ...
    form.push(this.aks212ASL);
    form.push(this.aks23ASG);

    form.push(this.aks310ASLDC);
    form.push(this.aks34ASGDC);
    form.push(this.aks311ASLIC);
    form.push(this.aks35ASGIC);

    form.push(this.aks48ASLDC);
    form.push(this.aks46ASGDC);
    form.push(this.aks49ASLIC);
    form.push(this.aks47ASGIC);
  ...
  seriNoKullanildiMiKontrol(seriNo: string) {
    if (this.aks11OSL.find(a => a.SeriNo === seriNo) === undefined
      ...
      && this.aks212ASL.find(a => a.SeriNo === seriNo) === undefined
      && this.aks23ASG.find(a => a.SeriNo === seriNo) === undefined

      && this.aks310ASLDC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks34ASGDC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks311ASLIC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks35ASGIC.find(a => a.SeriNo === seriNo) === undefined

      && this.aks48ASLDC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks46ASGDC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks49ASLIC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks47ASGIC.find(a => a.SeriNo === seriNo) === undefined
  ...
  lastikTipResim(lastikTipId: number) {
    if (lastikTipId === undefined) return "assets/aks-iskeletleri/8x4/img/tire.png"
    var lastikTip = this.lastikTipler.find(a => a.LastikTipID === lastikTipId);
    if (lastikTip !== undefined) {
      if (lastikTip.Ad === "Yeni") return "assets/aks-iskeletleri/8x4/img/yellow_tire.png"
      else if (lastikTip.Ad === "Kaplama") return "assets/aks-iskeletleri/8x4/img/blue_tire.png"
      else if (lastikTip.Ad === "Kullanılmış") return "assets/aks-iskeletleri/8x4/img/green_tire.png"
      else if (lastikTip.Ad === "OE") return "assets/aks-iskeletleri/8x4/img/red_tire.png"
      else return "assets/aks-iskeletleri/8x4/img/tire.png"
    }
    else {
