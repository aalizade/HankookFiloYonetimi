...
@Component({
  selector: 'app-kategori6x4',
  templateUrl: './kategori6x4.component.html',
  styleUrls: ['./kategori6x4.component.css']
})
export class Kategori6x4Component implements OnInit {
  ...
  aks29ASLDC: AracBakimlar[] = []; aks29ASLDCName: string = "29-A-SLDC"; aks29ASLDCId: number;
  aks23ASGDC: AracBakimlar[] = []; aks23ASGDCName: string = "23-A-SGDC"; aks23ASGDCId: number;
  aks210ASLIC: AracBakimlar[] = []; aks210ASLICName: string = "210-A-SLİC"; aks210ASLICId: number;
  aks24ASGIC: AracBakimlar[] = []; aks24ASGICName: string = "24-A-SGİC"; aks24ASGICId: number;

  aks37ASLDC: AracBakimlar[] = []; aks37ASLDCName: string = "37-A-SLDC"; aks37ASLDCId: number;
  aks35ASGDC: AracBakimlar[] = []; aks35ASGDCName: string = "35-A-SGDC"; aks35ASGDCId: number;
  aks38ASLIC: AracBakimlar[] = []; aks38ASLICName: string = "38-A-SLİC"; aks38ASLICId: number;
  aks36ASGIC: AracBakimlar[] = []; aks36ASGICName: string = "36-A-SGİC"; aks36ASGICId: number;
                ...
                if (aracKategoriResponse.Ad !== this.pageName) {
                  ...
                  this.aksPozisyonlarService.getAksPozisyonlar().toPromise().then(aksPozisyonResponse => {
                    ...
                    try {
                      ...
                      this.aks29ASLDCId = aksPozisyonResponse.find(a => a.Ad === this.aks29ASLDCName).AksPozisyonID;
                      this.aks23ASGDCId = aksPozisyonResponse.find(a => a.Ad === this.aks23ASGDCName).AksPozisyonID;
                      this.aks210ASLICId = aksPozisyonResponse.find(a => a.Ad === this.aks210ASLICName).AksPozisyonID;
                      this.aks24ASGICId = aksPozisyonResponse.find(a => a.Ad === this.aks24ASGICName).AksPozisyonID;

                      this.aks37ASLDCId = aksPozisyonResponse.find(a => a.Ad === this.aks37ASLDCName).AksPozisyonID;
                      this.aks35ASGDCId = aksPozisyonResponse.find(a => a.Ad === this.aks35ASGDCName).AksPozisyonID;
                      this.aks38ASLICId = aksPozisyonResponse.find(a => a.Ad === this.aks38ASLICName).AksPozisyonID;
                      this.aks36ASGICId = aksPozisyonResponse.find(a => a.Ad === this.aks36ASGICName).AksPozisyonID;
                      ...
                      this.aracBakimlarService.getAracBakimlarForAracId(response.AracID).subscribe(aracBakimResponse => {
                        ...
                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks29ASLDCId && a.Aktif === true) === undefined) this.aks29ASLDC.push();
                        else this.aks29ASLDC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks29ASLDCId && a.Aktif === true));
                        ...
                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks210ASLICId && a.Aktif === true) === undefined) this.aks210ASLIC.push();
                        else this.aks210ASLIC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks210ASLICId && a.Aktif === true));
                        ...
                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks37ASLDCId && a.Aktif === true) === undefined) this.aks37ASLDC.push();
                        else this.aks37ASLDC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks37ASLDCId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks35ASGDCId && a.Aktif === true) === undefined) this.aks35ASGDC.push();
                        else this.aks35ASGDC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks35ASGDCId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks38ASLICId && a.Aktif === true) === undefined) this.aks38ASLIC.push();
                        else this.aks38ASLIC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks38ASLICId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks36ASGICId && a.Aktif === true) === undefined) this.aks36ASGIC.push();
                        else this.aks36ASGIC.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks36ASGICId && a.Aktif === true));
  ...
  aracBakimIslemleriKaydet() {
    ...
    form.push(this.aks29ASLDC);
    form.push(this.aks23ASGDC);
    form.push(this.aks210ASLIC);
    form.push(this.aks24ASGIC);

    form.push(this.aks37ASLDC);
    form.push(this.aks35ASGDC);
    form.push(this.aks38ASLIC);
    form.push(this.aks36ASGIC);
  ...
  seriNoKullanildiMiKontrol(seriNo: string) {
    if (this.aks11OSL.find(a => a.SeriNo === seriNo) === undefined
      ...
      && this.aks29ASLDC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks23ASGDC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks210ASLIC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks24ASGIC.find(a => a.SeriNo === seriNo) === undefined

      && this.aks37ASLDC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks35ASGDC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks38ASLIC.find(a => a.SeriNo === seriNo) === undefined
      && this.aks36ASGIC.find(a => a.SeriNo === seriNo) === undefined
  ...
  lastikTipResim(lastikTipId: number) {
    if (lastikTipId === undefined) return "assets/aks-iskeletleri/6x4/img/tire.png"
    var lastikTip = this.lastikTipler.find(a => a.LastikTipID === lastikTipId);
    if (lastikTip !== undefined) {
      if (lastikTip.Ad === "Yeni") return "assets/aks-iskeletleri/6x4/img/yellow_tire.png"
      else if (lastikTip.Ad === "Kaplama") return "assets/aks-iskeletleri/6x4/img/blue_tire.png"
      else if (lastikTip.Ad === "Kullanılmış") return "assets/aks-iskeletleri/6x4/img/green_tire.png"
      else if (lastikTip.Ad === "OE") return "assets/aks-iskeletleri/6x4/img/red_tire.png"
      else return "assets/aks-iskeletleri/6x4/img/tire.png"
    }
    else {
