...
@Component({
  selector: 'app-kategori12treyler',
  templateUrl: './kategori12treyler.component.html',
  styleUrls: ['./kategori12treyler.component.css']
})
export class Kategori12treylerComponent implements OnInit {

  pageName: string = "12'li Treyler";
  depo_hide: boolean = true;

  depoList: AracBakimlar[] = [];
  hurdalikList: AracBakimlar[] = [];

  aks111ASLD: AracBakimlar[] = []; aks111ASLDName: string = "111-A-SLD"; aks111ASLDId: number;
  aks11ASGD: AracBakimlar[] = []; aks11ASGDName: string = "11-A-SGD"; aks11ASGDId: number;
  aks112ASLI: AracBakimlar[] = []; aks112ASLIName: string = "112-A-SLİ"; aks112ASLIId: number;
  aks12ASGI: AracBakimlar[] = []; aks12ASGIName: string = "12-A-SGİ"; aks12ASGIId: number;

  aks29ASLD: AracBakimlar[] = []; aks29ASLDName: string = "29-A-SLD"; aks29ASLDId: number;
  aks23ASGD: AracBakimlar[] = []; aks23ASGDName: string = "23-A-SGD"; aks23ASGDId: number;
  aks210ASLI: AracBakimlar[] = []; aks210ASLIName: string = "210-A-SLİ"; aks210ASLIId: number;
  aks24ASGI: AracBakimlar[] = []; aks24ASGIName: string = "24-A-SGİ"; aks24ASGIId: number;

  aks37ASLD: AracBakimlar[] = []; aks37ASLDName: string = "37-A-SLD"; aks37ASLDId: number;
  aks35ASGD: AracBakimlar[] = []; aks35ASGDName: string = "35-A-SGD"; aks35ASGDId: number;
  aks38ASLI: AracBakimlar[] = []; aks38ASLIName: string = "38-A-SLİ"; aks38ASLIId: number;
  aks36ASGI: AracBakimlar[] = []; aks36ASGIName: string = "36-A-SGİ"; aks36ASGIId: number;
              ...
              this.aracKategorilerService.getAracKategori(aracModelResponse.AracKategoriID).subscribe(async aracKategoriResponse => {
                if (aracKategoriResponse.Ad !== this.pageName) {
                  ...
                  this.aksPozisyonlarService.getAksPozisyonlar().toPromise().then(aksPozisyonResponse => {
                    ...
                    try {
                      this.aks111ASLDId = aksPozisyonResponse.find(a => a.Ad === this.aks111ASLDName).AksPozisyonID;
                      this.aks11ASGDId = aksPozisyonResponse.find(a => a.Ad === this.aks11ASGDName).AksPozisyonID;
                      this.aks112ASLIId = aksPozisyonResponse.find(a => a.Ad === this.aks112ASLIName).AksPozisyonID;
                      this.aks12ASGIId = aksPozisyonResponse.find(a => a.Ad === this.aks12ASGIName).AksPozisyonID;

                      this.aks29ASLDId = aksPozisyonResponse.find(a => a.Ad === this.aks29ASLDName).AksPozisyonID;
                      this.aks23ASGDId = aksPozisyonResponse.find(a => a.Ad === this.aks23ASGDName).AksPozisyonID;
                      this.aks210ASLIId = aksPozisyonResponse.find(a => a.Ad === this.aks210ASLIName).AksPozisyonID;
                      this.aks24ASGIId = aksPozisyonResponse.find(a => a.Ad === this.aks24ASGIName).AksPozisyonID;

                      this.aks37ASLDId = aksPozisyonResponse.find(a => a.Ad === this.aks37ASLDName).AksPozisyonID;
                      this.aks35ASGDId = aksPozisyonResponse.find(a => a.Ad === this.aks35ASGDName).AksPozisyonID;
                      this.aks38ASLIId = aksPozisyonResponse.find(a => a.Ad === this.aks38ASLIName).AksPozisyonID;
                      this.aks36ASGIId = aksPozisyonResponse.find(a => a.Ad === this.aks36ASGIName).AksPozisyonID;
                      ...
                      this.aracBakimlarService.getAracBakimlarForAracId(response.AracID).subscribe(aracBakimResponse => {
                        ...
                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks111ASLDId && a.Aktif === true) === undefined) this.aks111ASLD.push();
                        else this.aks111ASLD.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks111ASLDId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks11ASGDId && a.Aktif === true) === undefined) this.aks11ASGD.push();
                        else this.aks11ASGD.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks11ASGDId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks112ASLIId && a.Aktif === true) === undefined) this.aks112ASLI.push();
                        else this.aks112ASLI.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks112ASLIId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks12ASGIId && a.Aktif === true) === undefined) this.aks12ASGI.push();
                        else this.aks12ASGI.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks12ASGIId && a.Aktif === true));



                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks29ASLDId && a.Aktif === true) === undefined) this.aks29ASLD.push();
                        else this.aks29ASLD.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks29ASLDId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks23ASGDId && a.Aktif === true) === undefined) this.aks23ASGD.push();
                        else this.aks23ASGD.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks23ASGDId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks210ASLIId && a.Aktif === true) === undefined) this.aks210ASLI.push();
                        else this.aks210ASLI.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks210ASLIId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks24ASGIId && a.Aktif === true) === undefined) this.aks24ASGI.push();
                        else this.aks24ASGI.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks24ASGIId && a.Aktif === true));



                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks37ASLDId && a.Aktif === true) === undefined) this.aks37ASLD.push();
                        else this.aks37ASLD.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks37ASLDId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks35ASGDId && a.Aktif === true) === undefined) this.aks35ASGD.push();
                        else this.aks35ASGD.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks35ASGDId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks38ASLIId && a.Aktif === true) === undefined) this.aks38ASLI.push();
                        else this.aks38ASLI.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks38ASLIId && a.Aktif === true));

                        if (aracBakimResponse.find(a => a.AksPozisyonID === this.aks36ASGIId && a.Aktif === true) === undefined) this.aks36ASGI.push();
                        else this.aks36ASGI.push(aracBakimResponse.find(a => a.AksPozisyonID === this.aks36ASGIId && a.Aktif === true));
  ...
  aracBakimIslemleriKaydet() {
    ...
    form.push(this.aks111ASLD);
    form.push(this.aks11ASGD);
    form.push(this.aks112ASLI);
    form.push(this.aks12ASGI);

    form.push(this.aks29ASLD);
    form.push(this.aks23ASGD);
    form.push(this.aks210ASLI);
    form.push(this.aks24ASGI);

    form.push(this.aks37ASLD);
    form.push(this.aks35ASGD);
    form.push(this.aks38ASLI);
    form.push(this.aks36ASGI);
  ...
  seriNoKullanildiMiKontrol(seriNo: string) {
    if (this.aks111ASLD.find(a => a.SeriNo === seriNo) === undefined
      && this.aks11ASGD.find(a => a.SeriNo === seriNo) === undefined
      && this.aks112ASLI.find(a => a.SeriNo === seriNo) === undefined
      && this.aks12ASGI.find(a => a.SeriNo === seriNo) === undefined

      && this.aks29ASLD.find(a => a.SeriNo === seriNo) === undefined
      && this.aks23ASGD.find(a => a.SeriNo === seriNo) === undefined
      && this.aks210ASLI.find(a => a.SeriNo === seriNo) === undefined
      && this.aks24ASGI.find(a => a.SeriNo === seriNo) === undefined

      && this.aks37ASLD.find(a => a.SeriNo === seriNo) === undefined
      && this.aks35ASGD.find(a => a.SeriNo === seriNo) === undefined
      && this.aks38ASLI.find(a => a.SeriNo === seriNo) === undefined
      && this.aks36ASGI.find(a => a.SeriNo === seriNo) === undefined
  ...
  lastikTipResim(lastikTipId: number) {
    if (lastikTipId === undefined) return "assets/aks-iskeletleri/12li_treyler/img/tire.png"
    var lastikTip = this.lastikTipler.find(a => a.LastikTipID === lastikTipId);
    if (lastikTip !== undefined) {
      if (lastikTip.Ad === "Yeni") return "assets/aks-iskeletleri/12li_treyler/img/yellow_tire.png"
      else if (lastikTip.Ad === "Kaplama") return "assets/aks-iskeletleri/12li_treyler/img/blue_tire.png"
      else if (lastikTip.Ad === "Kullanılmış") return "assets/aks-iskeletleri/12li_treyler/img/green_tire.png"
      else if (lastikTip.Ad === "OE") return "assets/aks-iskeletleri/12li_treyler/img/red_tire.png"
      else return "assets/aks-iskeletleri/12li_treyler/img/tire.png"
    }
    else {
